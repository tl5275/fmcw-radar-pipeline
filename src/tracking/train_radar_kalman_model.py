#!/usr/bin/env python3
"""Train a CNN + physics + Kalman radar micro-Doppler classifier."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a radar classifier with CNN, physics, and Kalman motion features.")
    ...
    parser.add_argument(
        "--model-path",
        default="cnn_physics_kalman_model.pth",
...
def compute_physics_features_from_image(image: np.ndarray) -> np.ndarray:
    """Fallback extractor used when an image is not already in the CSV."""
...
def extract_doppler_ridge(spectrogram: np.ndarray) -> np.ndarray:
    """Track the strongest Doppler bin for each time column."""
    return np.argmax(spectrogram, axis=0).astype(np.float32)
...
def kalman_filter_1d(
    ...
    """Apply a scalar Kalman filter to smooth the Doppler ridge trajectory."""
    if measurements.size == 0:
        return np.array([], dtype=np.float32)

    estimate = float(measurements[0])
    covariance = 1.0
    smoothed = []

    for measurement in measurements:
        # Prediction: assume constant velocity/ridge state.
        predicted_estimate = estimate
        predicted_covariance = covariance + process_noise

        # Update: incorporate the new ridge measurement.
        innovation = float(measurement) - predicted_estimate
...
def compute_kalman_motion_features(spectrogram: np.ndarray) -> np.ndarray:
    """Extract motion descriptors from the Kalman-smoothed Doppler ridge."""
    ridge = extract_doppler_ridge(spectrogram)
    smoothed_velocity = kalman_filter_1d(ridge)
    if smoothed_velocity.size == 0:
        return np.zeros(4, dtype=np.float32)

    acceleration = np.diff(smoothed_velocity)
    mean_velocity = float(np.mean(smoothed_velocity))
    velocity_variance = float(np.var(smoothed_velocity))
    acceleration_variance = float(np.var(acceleration)) if acceleration.size else 0.0
    trajectory_smoothness = float(np.mean(np.abs(np.diff(smoothed_velocity)))) if smoothed_velocity.size > 1 else 0.0

    return np.array(
        [mean_velocity, velocity_variance, acceleration_variance, trajectory_smoothness],
        dtype=np.float32,
    )


class RadarKalmanDataset(Dataset):
    """Return image tensor, standardized physics features, Kalman features, and label."""
    ...
    def __init__(
        ...
        self.raw_physics_features = self._build_physics_feature_matrix()
        self.raw_kalman_features = self._build_kalman_feature_matrix()
        self.scaled_physics_features = self.raw_physics_features.copy()
        self.scaled_kalman_features = self.raw_kalman_features.copy()
    ...
    def _load_normalized_image(self, image_path: str) -> np.ndarray:
        image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
        if image is None:
            raise RuntimeError(f"Failed to read image: {image_path}")
        return image.astype(np.float32) / 255.0

    def _build_physics_feature_matrix(self) -> np.ndarray:
        ...
        for image_path, _ in self.samples:
            ...
            # The CSV currently covers the synthetic training set. For real test
            # images we compute the same 5 physics features on the fly.
            normalized = self._load_normalized_image(image_path)
            feature_rows.append(compute_physics_features_from_image(normalized))

        return np.stack(feature_rows).astype(np.float32)

    def _build_kalman_feature_matrix(self) -> np.ndarray:
        feature_rows: list[np.ndarray] = []
        for image_path, _ in self.samples:
            normalized = self._load_normalized_image(image_path)
            feature_rows.append(compute_kalman_motion_features(normalized))
        return np.stack(feature_rows).astype(np.float32)

    def apply_feature_scalers(self, physics_scaler: StandardScaler, kalman_scaler: StandardScaler) -> None:
        self.scaled_physics_features = physics_scaler.transform(self.raw_physics_features).astype(np.float32)
        self.scaled_kalman_features = kalman_scaler.transform(self.raw_kalman_features).astype(np.float32)
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, int]:
        ...
        kalman_features = torch.from_numpy(self.scaled_kalman_features[index])
        return image_tensor, physics_features, kalman_features, label


class CNNPhysicsKalmanModel(nn.Module):
    """CNN image encoder fused with 5 physics and 4 Kalman motion features."""
    ...
    def forward(
        ...
        fused_features = torch.cat([cnn_features, physics_features, kalman_features], dim=1)
        return self.classifier(fused_features)


def create_dataloaders(
    args: argparse.Namespace,
) -> tuple[RadarKalmanDataset, RadarKalmanDataset, DataLoader, DataLoader, StandardScaler, StandardScaler]:
    transform = build_transform()
    feature_lookup = load_feature_lookup(args.feature_csv)

    train_dataset = RadarKalmanDataset(root_dir=args.train_root, feature_lookup=feature_lookup, transform=transform)
    test_dataset = RadarKalmanDataset(root_dir=args.test_root, feature_lookup=feature_lookup, transform=transform)

    # Standardize both handcrafted branches on training data only.
    physics_scaler = StandardScaler()
    physics_scaler.fit(train_dataset.raw_physics_features)
    kalman_scaler = StandardScaler()
    kalman_scaler.fit(train_dataset.raw_kalman_features)

    train_dataset.apply_feature_scalers(physics_scaler, kalman_scaler)
    test_dataset.apply_feature_scalers(physics_scaler, kalman_scaler)
    ...
    return train_dataset, test_dataset, train_loader, test_loader, physics_scaler, kalman_scaler


def train_one_epoch(
    model: CNNPhysicsKalmanModel,
    ...
    for image_tensor, physics_features, kalman_features, labels in dataloader:
        image_tensor = image_tensor.to(device)
        physics_features = physics_features.to(device)
        kalman_features = kalman_features.to(device)
        labels = labels.to(device)

        optimizer.zero_grad(set_to_none=True)
        logits = model(image_tensor, physics_features, kalman_features)
...
def save_model(
    ...
    checkpoint = {
        ...
        "kalman_columns": KALMAN_COLUMNS,
        "image_size": IMAGE_SIZE,
        "physics_scaler_mean": physics_scaler.mean_,
        "physics_scaler_scale": physics_scaler.scale_,
        "kalman_scaler_mean": kalman_scaler.mean_,
        "kalman_scaler_scale": kalman_scaler.scale_,
    }
...
def main() -> int:
    ...
    try:
        train_dataset, test_dataset, train_loader, test_loader, physics_scaler, kalman_scaler = create_dataloaders(args)
    ...
    model = CNNPhysicsKalmanModel(num_classes=len(CLASS_NAMES)).to(device)
