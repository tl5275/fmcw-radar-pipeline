#!/usr/bin/env python3
"""Train a temporal hybrid radar classifier using CNN, LSTM, and physics features."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a temporal CNN-LSTM radar classifier fused with physics features."
    )
    ...
    parser.add_argument(
        "--model-path",
        default="temporal_physics_radar_model.pth",
...
def build_transform() -> transforms.Compose:
    """Resize spectrograms and normalize grayscale intensities."""
...
def compute_physics_features_from_image(image: np.ndarray) -> np.ndarray:
    """Fallback extractor for images that are not already in the feature CSV."""
...
class RadarSequenceDataset(Dataset):
    """Return a sequence of image slices, standardized physics features, and label."""
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor, int]:
        ...
        # Split the resized spectrogram into vertical slices so the LSTM can
        # model temporal evolution along the time axis.
        frames = torch.chunk(image_tensor, chunks=SEQUENCE_LENGTH, dim=2)
        sequence_tensor = torch.stack(frames, dim=0)
        physics_features = torch.from_numpy(self.scaled_physics_features[index])
        return sequence_tensor, physics_features, label


class TemporalPhysicsRadarModel(nn.Module):
    """Per-frame CNN encoder + LSTM temporal model + physics feature fusion."""
    ...
    def forward(self, sequence_tensor: torch.Tensor, physics_features: torch.Tensor) -> torch.Tensor:
        batch_size, sequence_length, channels, height, width = sequence_tensor.shape
        frames = sequence_tensor.view(batch_size * sequence_length, channels, height, width)
        cnn_features = self.frame_cnn(frames)
        cnn_features = self.frame_projection(cnn_features)
        cnn_features = cnn_features.view(batch_size, sequence_length, -1)

        lstm_output, (hidden_state, _) = self.lstm(cnn_features)
        temporal_features = hidden_state[-1]
        fused_features = torch.cat([temporal_features, physics_features], dim=1)
        return self.classifier(fused_features)


def create_dataloaders(
    args: argparse.Namespace,
) -> tuple[RadarSequenceDataset, RadarSequenceDataset, DataLoader, DataLoader, StandardScaler]:
    transform = build_transform()
    feature_lookup = load_feature_lookup(args.feature_csv)

    train_dataset = RadarSequenceDataset(root_dir=args.train_root, feature_lookup=feature_lookup, transform=transform)
    test_dataset = RadarSequenceDataset(root_dir=args.test_root, feature_lookup=feature_lookup, transform=transform)
...
def train_one_epoch(
    model: TemporalPhysicsRadarModel,
    ...
    for sequence_tensor, physics_features, labels in dataloader:
        sequence_tensor = sequence_tensor.to(device)
        physics_features = physics_features.to(device)
        labels = labels.to(device)

        optimizer.zero_grad(set_to_none=True)
        logits = model(sequence_tensor, physics_features)
...
def save_model(model: TemporalPhysicsRadarModel, scaler: StandardScaler, output_path: str) -> None:
    checkpoint = {
...
def main() -> int:
    ...
    try:
        train_dataset, test_dataset, train_loader, test_loader, scaler = create_dataloaders(args)
    ...
    model = TemporalPhysicsRadarModel(num_classes=len(CLASS_NAMES)).to(device)
    ...
    save_model(model=model, scaler=scaler, output_path=args.model_path)
    return 0
